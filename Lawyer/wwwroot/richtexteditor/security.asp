...
<%
    ...
    If len(typ)>0 Then
		Select case typ
            ...
            case "John"
                rte.SecurityPolicyFile = "admin.config"
                store = "~/uploads/Users/John"
                rte.SetSecurity "Gallery", "*", "StoragePath", store
                policyfiletext = "Policy file:"  & rte.SecurityPolicyFile+ "<br/> Gallery:" & store
            case "Mary"
                rte.SecurityPolicyFile = "default.config"
                store = "~/uploads/Users/Mary"
                rte.SetSecurity "Gallery", "*", "StoragePath", store
                policyfiletext = "Policy file:" & rte.SecurityPolicyFile+ "<br/> Gallery:" & store
            case "Tim"
                rte.SecurityPolicyFile = "default.config"
                store = "~/uploads/Users/Tim"
                rte.SetSecurity "Gallery", "*", "StoragePath", store
                policyfiletext = "Policy file:" & rte.SecurityPolicyFile+ "<br/> Gallery:" & store
        End Select
    End If
